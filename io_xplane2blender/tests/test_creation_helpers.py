...
def create_datablock_mesh(
    ...
    # Resolve the parent object reference by name BEFORE ob.name is reassigned.
    # Blender evicts the existing owner of a name when ob.name is set, so if the
    # parent empty and this mesh share the same name the lookup done inside
    # set_parent would return ob itself, triggering "cannot assign to itself".
    if info.parent_info is not None and isinstance(info.parent_info.parent, str):
        resolved = bpy.data.objects.get(info.parent_info.parent)
        if resolved is not None:
            info.parent_info.parent = resolved
