            ...
            if self.type == MANIP_DRAG_XY:
                ...
                * (guarenteed by get_tranlation_bone)
                ...
                if translation_bone is None:
                    ...
                    v2_min = _trans_table[0][0]
                    v2_max = _trans_table[-1][0]
            ...
            if (
                ...
                def validate_axis_detent_ranges(
                    ...
                    if not axis_detent_ranges[0].start == v1_min:
                        logger.warn(
                            "Axis detent range list for {} does not start at Dataref 1's minimum value {} (range starts at {}, may be valid for imported OBJs)".format(
                                translation_bone.getBlenderName(), v1_min, axis_detent_ranges[0].start
                            )
                        )

                    if not axis_detent_ranges[-1].end == v1_max:
                        logger.warn(
                            "Axis detent range list for {} does not end at Dataref 1's maximum value {} (ends at {}, may be valid for imported OBJs)".format(
                                translation_bone.getBlenderName(), v1_max, axis_detent_ranges[-1].end
                            )
                        )
                    ...
                    for i in range(len(axis_detent_ranges)):
                        ...
                        if not 0.0 <= detent_range.height:
                            logger.error(
                                "Height in axis detent range {} on {} must be >= 0.0".format(
                                    detent_range,
                                    translation_bone.getBlenderName(),
                                )
                            )
                        ...
                        if not detent_range.end == detent_range_next.start:
                            logger.warn(
                                "In {}'s axis detent range list, the start of a detent range does not equal the end of the previous detent range {},{}  (may be valid for imported OBJs)".format(
                                    translation_bone.getBlenderName(),
                                    detent_range,
                                    (
                        ...
                        if (
                            ...
                            logger.warn(
                                "Stop pit created by {}'s detent range {} is not lower than"
                                " previous {} and next detent ranges {} (may be valid for imported OBJs)".format(
                                    translation_bone.getBlenderName(),
                                    (detent_range),
                                    (
